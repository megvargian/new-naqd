...
function wp_bootstrap_starter_scripts()
{
    ...
    // load bootstrap js
    wp_enqueue_script('wp-bootstrap-starter-popper', get_template_directory_uri() . '/inc/assets/js/popper.min.js', array(), '6.8.8', false);
    wp_enqueue_script('wp-bootstrap-starter-bootstrapjs', get_template_directory_uri() . '/inc/assets/js/bootstrap.min.js', array(), '6.8.8', false);
...
function load_more_trend_posts() {
    $page = $_POST['page'];
...
add_action('wp_ajax_load_more_trend_posts', 'load_more_trend_posts');
add_action('wp_ajax_nopriv_load_more_trend_posts', 'load_more_trend_posts');
...
function load_more_authors() {
    ...
    if ($query->have_posts()) :
        ...
        $get_fields = get_fields($author_id);
    ?>
        <div class="col-lg-3 col-4 mb-sm-4 mb-2">
            <a href="<?php echo get_permalink($author_id); ?>">
                <div class="author-profile p-sm-4 p-2">
                    <?php if(wp_get_attachment_url( get_post_thumbnail_id($author_id))){?>
                        <img class="rounded-circle" src="<?php echo wp_get_attachment_url( get_post_thumbnail_id($author_id)); ?>" alt="<?php echo $author_title; ?>">
                    <?php } else { ?>
                        <img class="rounded-circle" src="<?php echo get_template_directory_uri(); ?>/inc/assets/images/big-avatar.png" alt="<?php echo $author_title; ?>">
                    <?php } ?>
                    <h2><?php echo $author_title; ?></h2>
                    <p><?php echo $get_fields['title']; ?></p>
                </div>
            </a>
        </div>
    <?php
...
add_action('wp_ajax_load_more_authors', 'load_more_authors');
add_action('wp_ajax_nopriv_load_more_authors', 'load_more_authors');
...
function search_authors() {
    ...
    $args = array(
        'post_type'      => 'authors', // Your custom post type
        'posts_per_page' => -1,
        'post_status'    => 'publish',
        'order'          => 'DESC',
        'orderby'        => 'date',
...
add_action('wp_ajax_search_authors', 'search_authors');
add_action('wp_ajax_nopriv_search_authors', 'search_authors');

function search_all_authors() {
    $args = array(
        'post_type'      => 'authors', // Your custom post type
        'posts_per_page' => -1,
        'post_status'    => 'publish',
...
add_action('wp_ajax_search_all_authors', 'search_all_authors');
add_action('wp_ajax_nopriv_search_all_authors', 'search_all_authors');
